...
const firebaseConfig = {
  ...
  storageBucket: "solimedical-micu.appspot.com",
  messagingSenderId: "267176898574",
  appId: "1:267176898574:web:solimedicalmicu"
};
...
try {
  ...
  messaging.onBackgroundMessage((payload) => {
    ...
    const title = payload.notification?.title || payload.data?.title || 'Soli Medical MICU';
    const body = payload.notification?.body || payload.data?.body || 'Clinical Notification';
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    if (data.notification || data.data) {
      const title = data.notification?.title || data.data?.title || 'Soli Medical MICU';
      const body = data.notification?.body || data.data?.body || 'Clinical Notification';
      const options = {
        body: body,
        icon: '/pwa-192x192.png',
        badge: '/favicon-32x32.png',
