...
export const BedMatrixCard: React.FC<BedMatrixCardProps> = ({
  ...
  const isOccupied = bed.status === BedStatus.OCCUPIED && patient;
  ...
  const isVacant = bed.status === BedStatus.VACANT || (!patient && !isDecontaminating);
  ...
  const activePumps = pumps.filter(p => p.status === 'RUNNING' || p.status === 'TITRATING');
  ...
  return (
    ...
    >
      ...
      {/* Main Body: Patient Information or Vacant Prompt */}
          ...
          <>
            ...
            <div>
              ...
              <div className="text-[10px] text-slate-400 mt-1 flex items-center gap-2">
                <span>{patient?.age} {lang === 'ar' ? 'سنة' : 'yo'}</span>
                <span>•</span>
                <span>{patient?.gender === 'MALE' ? (lang === 'ar' ? 'ذكر' : 'Male') : (lang === 'ar' ? 'أنثى' : 'Female')}</span>
                <span>•</span>
                <span>{lang === 'ar' ? `الوزن: ${patient?.weightKg} كجم` : `Weight: ${patient?.weightKg} kg`} (IBW: {patient?.idealBodyWeightKg})</span>
              </div>
            </div>
        ...
        {isOccupied || isTransferPending ? (
          <>
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-xl bg-teal-500/10 hover:bg-teal-500/20 text-teal-300 border border-teal-500/30 text-xs font-bold transition-all active:scale-95"
            >
            ...
            {settings.features.enableTelemetryVitals && (
              <button
                ...
                className="flex items-center justify-center p-2 rounded-xl bg-[#0e172a] hover:bg-[#1e293b] text-slate-300 border border-slate-700 text-xs transition-all active:scale-95"
          ...
          settings.features.enableAdmissions && (
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-500 hover:to-teal-400 text-slate-950 text-xs font-bold transition-all active:scale-95 shadow-md"
            >
